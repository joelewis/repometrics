...
export default {
    ...
    watch: {
        ...
        search: function(val) {
            ...
            fetch('https://api.github.com/search/repositories?q='+query+'&sort=stars', {// TOTHINK: convert to JS template strings?
                headers: {
        ...
        selectedProjects: function(selectedProjects) {
            ...
            Promise.all(
                projects.map(project =>
                    fetch(`https://api.github.com/repos/${project.full_name}/stats/commit_activity`, {
                        headers: {
                            'Authorization': `Basic ${btoa(this.username+':'+this.password)}`
                        }
                    }).then(resp => resp.json())
    ...
    methods: {
        ...
        getRepoForId: function(id) {
            ...
            throw new Error(`github repo with id ${id} cannot be found in cached items`);
        }
    }
  }
</script>